...
public class LinkedList
{
    private class Node
    {
        ...
        public Node(int data)
        {
            ...
            Prev = null; // По умолчанию предыдущий узел не установлен
        }
    }

    private Node head; // Голова списка, первый узел
    private int count; // Количество узлов в списке
    private Node[] indexArray; // Массив для хранения ссылок узлов на каждый 50-ый узел

    public LinkedList()
    {
        head = null; // Инициализация пустого списка
        count = 0;
        indexArray = null; // Инициализация массива для индекса
    }
    ...
    public void Add(int data)
    {
        ...
        else
        {
            ...
            current.Next.Prev = current; // Устанавливаем ссылку на предыдущий узел
        }
        count++;
    }

    // Метод для создания массива с ссылками на каждый 50-ый узел
    public void CreateIndex()
    {
        int indexSize = count / 50 + 1; // Определяем размер массива для индекса
        indexArray = new Node[indexSize];
        ...
        while (current != null)
        {
            if (indexCounter % 50 == 0)
            {
                indexArray[indexCounter / 50] = current; // Записываем ссылку на текущий узел в массив индекса
            }
            current = current.Next;
            indexCounter++;
        }

        Console.WriteLine("Индекс успешно создан.");
    }

    // Метод для двунаправленного поиска вперед
    public Node FindForward(int data)
    {
        Node current = head;
        while (current != null)
        {
    ...
    // Метод для двунаправленного поиска назад
    public Node FindBackward(int data)
    {
        if (indexArray == null)
        {
            Console.WriteLine("Индекс не создан, невозможно выполнить двунаправленный поиск.");
            return null;
        }

        for (int i = indexArray.Length - 1; i >= 0; i--)
        {
            Node current = indexArray[i];
            while (current != null)
            {
                if (current.Data == data)
                {
